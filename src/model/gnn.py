...
class GNNModel(nn.Module):
    ...
    def forward(self, data: ModelInput, boundary: torch.Tensor, layer_idx: int = -1):
        if not data.is_ent:
            ...
            representations = self.relation_model(
                data.data[0], boundary=boundary[0].unsqueeze(0), layer_idx=layer_idx
            )
        else:
            # ent gnn
            ...
            if layer_idx == 0:
                super_edge_emb = torch.sum(
                    data.rel_emb, dim=1, keepdim=True
                )  # sum / mean
                if data.rel_emb.shape[1] > data.data[0].super_edge_type[1]:
                    ...
                    # 覆盖和添加
                    num_gap = (
                        data.data[0].super_edge_type[1] + 1 - data.rel_emb.shape[1]
                    )
                    rel_emb = torch.cat(
                        [
                            data.rel_emb,
                            torch.zeros(
                                1,
                                num_gap,
                                data.rel_emb.shape[2],
                                device=data.rel_emb.device,
                                dtype=data.rel_emb.dtype,
                            ),
                        ],
                        dim=1,
                    )
            ...
            representations = self.entity_model(
                data.data[0], data.rel_emb, data.mask_triples, boundary, layer_idx
            )
